import type {SnippetVariantProps, SnippetSlots, SlotsToClasses} from "@nexus-ds/theme";

import {snippet} from "@nexus-ds/theme";
...
import {TooltipProps} from "@nexus-ds/tooltip";
import {ButtonProps} from "@nexus-ds/button";
export interface UseSnippetProps extends Omit<HTMLnexusuiProps, "onCopy">, SnippetVariantProps {
  /**
   * Ref to the DOM node.
   */
  ...
  /**
   * Tooltip props.
   * @see [Tooltip](https://nexusui.org/components/tooltip) for more details.
   * @default {
   *  offset: 15,
   *  delay: 1000,
  ...
  /**
   * Copy button props.
   * @see [Button](https://nexusui.org/components/button) for more details.
   * @default {
   *   isDisabled: disableCopy,
   *   onPress: onCopy
   *   size:"sm",
