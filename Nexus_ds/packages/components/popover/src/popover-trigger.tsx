...
import {pickChildren} from "@nexus-ds/react-utils";
...
const PopoverTrigger = forwardRef<"button", PopoverTriggerProps>((props, _) => {
  ...
  // validates if contains a nexusui Button as a child
  const [, triggerChildren] = pickChildren(children, Button);

  const {buttonProps} = useAriaButton({onPress}, triggerRef);

  const hasnexusuiButton = useMemo<boolean>(() => {
    return triggerChildren?.[0] !== undefined;
  }, [triggerChildren]);

  return cloneElement(child, mergeProps(rest, hasnexusuiButton ? {onPress} : buttonProps));
});

PopoverTrigger.displayName = "nexusui.PopoverTrigger";
