...
import {ConfigTheme, ConfigThemes, DefaultThemeType, nexusuiPluginConfig} from "./types";
import {lightLayout, darkLayout, defaultLayout} from "./default-layout";
import {baseStyles} from "./utils/classes";

const DEFAULT_PREFIX = "nexusui";
  ...
  forEach(themes, ({extend, layout, colors}: ConfigTheme, themeName: string) => {
    ...
    forEach(flatColors, (colorValue, colorName) => {
      ...
      try {
        ...
        const nexusuiColorVariable = `--${prefix}-${colorName}`;
        const nexusuiOpacityVariable = `--${prefix}-${colorName}-opacity`;

        // set the css variable in "@layer utilities"
        resolved.utilities[cssSelector]![nexusuiColorVariable] = `${h} ${s}% ${l}%`;
        // if an alpha value was provided in the color definition, store it in a css variable
        if (typeof defaultAlphaValue === "number") {
          resolved.utilities[cssSelector]![nexusuiOpacityVariable] = defaultAlphaValue.toFixed(2);
        }
        // set the dynamic color in tailwind config theme.colors
        resolved.colors[colorName] = ({opacityVariable, opacityValue}) => {
          // if the opacity is set  with a slash (e.g. bg-primary/90), use the provided value
          if (!isNaN(+opacityValue)) {
            return `hsl(var(${nexusuiColorVariable}) / ${opacityValue})`;
          }
          // if no opacityValue was provided (=it is not parsable to a number)
          // the nexusuiOpacityVariable (opacity defined in the color definition rgb(0, 0, 0, 0.5)) should have the priority
          // over the tw class based opacity(e.g. "bg-opacity-90")
          // This is how tailwind behaves as for v3.2.4
          if (opacityVariable) {
            return `hsl(var(${nexusuiColorVariable}) / var(${nexusuiOpacityVariable}, var(${opacityVariable})))`;
          }

          return `hsl(var(${nexusuiColorVariable}) / var(${nexusuiOpacityVariable}, 1))`;
        };
      } catch (error: any) {
...
export const nexusui = (config: nexusuiPluginConfig = {}): ReturnType<typeof plugin> => {
  const {
    themes: themeObject = {},
    defaultTheme = "light",
    layout: userLayout,
